...
def resize(path, new_horizontal_size = None, new_vertical_size = None):
	try:
		...
		vsize = int((flost(height_img)) * float((new_horizontal_size / float(width_img)))) if new_vertical_size is None else new_vertical_size
		...
		img.save(save_path)
				...
				#		img = img.resize((width_img, new_height), Image.ANTIALIAS)
...
parser = ArgumentParser(description = "Programm that resizes an image to a custom size.")
